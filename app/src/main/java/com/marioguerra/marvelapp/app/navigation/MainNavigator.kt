...
class MainNavigator(private val fragmentManager: FragmentManager, private val containerId: Int) {

    fun addFragment(fragment: BaseFragment, addToBackStack: Boolean, tag: String?) {
        val transaction = fragmentManager.beginTransaction().add(containerId, fragment)
        if (addToBackStack) {
